...
def check_tests_for_host(host, mesh_config):
    """
    Classifies the host as belonging to one of the two test groups (latency and trace).
    """
    try:
...
def create_hosts_tests_types_grid(hosts, mesh_config):
    ...
    host_test_type.columns = ['host', 'owd', 'trace']
...
if __name__ == '__main__':
    ...
    now = dt.datetime.now()
    time_from = (now - dt.timedelta(hours=12)).strftime('%Y-%m-%d %H:%M')
    time_to = (now - dt.timedelta(hours=10)).strftime('%Y-%m-%d %H:%M')
    # print(time_from, time_to)
    tests = {'owd': {'dataFrom': time_from, 'dataTo': time_to, 'title': 'LATENCY'}, 'trace': {'dataFrom': time_from, 'dataTo': time_to, 'title': 'TRACE'}}
    for test in tests.keys():
        alarmOnMulty1 = alarms('Networking', 'Sites', f"tests' results for hosts not found in ps_{test}")
        # line = '------------------------------------------------------------------'
        # print(line)
